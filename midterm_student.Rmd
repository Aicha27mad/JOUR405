...
Name: Mamaissata Aicha Bangoura
...
```{r}
restaurant_inspect <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/montco_inspections.csv")
...
```{r}
restaurant_inspect |> summarize (mean = mean(compliance_score), sd =sd(compliance_score))

restaurant_inspect |> 
  ggplot()+
  geom_point(aes(x=compliance_score, y=compliance_score))

Answer: Theres a positive correlation in this distribution. 
...
```{r}
HighSchool_Athletics <- read_csv ("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/md_hs_participation.csv") 
```
...
```{r}
HighSchool_Athletics |> summarize(corelation =cor(boys,girls, method ="pearson"))
...
```{r}
PublicTransit <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/wmata_daily.csv")
...
```{r}
PublicTransit |> summarize (mean = mean(bus), sd =sd(rail)) 


Weekdaymeans <- PublicTransit |>
  group_by(weekday) |>
  summarize(mean = mean(bus), sd =sd(rail))

Thursday stands out as the mean and standard deviation are the largest, whihc means theres more variation. The values are spead out. 
...
```{r}
Marylandcarthefts <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/md_car_thefts.csv")
```
...
```{r}
Marylandcarthefts|> mutate(total_rate = 2023 /population*10000)

Marylandcarthefts|> mutate(median_rate = 2023 /population*10000)
...
ANSWER HERE: 
I could search for data that prove causation or correlation to why this is. Scatterplots could help readers understand the trends. I may need sources who called for the service and recive a firsthand witness statement. 
