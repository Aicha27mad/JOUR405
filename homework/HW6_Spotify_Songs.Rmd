...
```{r}
  geom_vline(aes(xintercept = mean(danceabilty)), color ="red", linetype = "dashed", size = 1) +
...
```{r}
The distribution is not normal. It is skewed the right direction. The mean tells us that the danceabilty score is fairly high. and the standard deviation tells us thats the average songs of danceabilty being is less or more than the mean. A story to cover with the data would be the type of genres or atrists people on in todays day in age listen to to dance. One question to report on could be what in the songs stimulate their energy to get up and dance.
```
